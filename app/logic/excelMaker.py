...
class ExcelMaker:
    ...
    def writeAllRoomHeaders(self, sheet):
        ...
        sheet.write('D1', 'Visual Accessibility')
        sheet.write('E1', 'Audio Accessibility')
        sheet.write('F1', 'Physical Accessibility')
        sheet.write('G1', 'Specialized Equipment')
        sheet.write('H1', 'Special Features')
        sheet.write('I1', 'Movable Furniture')
        sheet.write('J1', 'Projector(s)')
        sheet.write('K1', 'SmartBoard(s)')
        sheet.write('L1', 'Instructor Computers')
        sheet.write('M1', 'Podium')
        sheet.write('N1', 'Student Workspaces')
        sheet.write('O1', 'Chalkboards')
        sheet.write('P1', 'WhiteBoards')
        sheet.write('Q1', 'DVD')
        sheet.write('R1', 'Blu-Ray')
        sheet.write('S1', 'Audio')
        sheet.write('T1', 'Extro')
        sheet.write('U1', 'Doc Cam')
        sheet.write('V1', 'VHS')
        sheet.write('W1', 'Mondopad')
        sheet.write('X1', 'Tech Cart')
        self.intr_letter = 'X'
    
        
    def write_all_rooms_info(self, sheet, room, row):
        ...
        sheet.write('D{0}'.format(row), room.visualAcc)
        sheet.write('E{0}'.format(row), room.audioAcc)
        sheet.write('F{0}'.format(row), room.physicalAcc)
    ...
    def make_master_file(self,term):
        ...
        self.writeRoomHeaders(allrooms_sheet, 'ALL')
        
        availablerooms_sheet = workbook.add_worksheet('Available Rooms')
        self.writeRoomHeaders(availablerooms_sheet, 'available')
        
        rooms = Rooms.select().order_by(Rooms.building_id)
        for room in rooms:
            self.write_all_rooms_info(allrooms_sheet, room, self.room_row)
            self.room_row += 1
