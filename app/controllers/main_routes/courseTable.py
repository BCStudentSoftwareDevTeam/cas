...
@main_bp.route("/courses/<prefix>/", methods=["GET", "POST"])
def retrieveCourses(prefix):

    au = AuthorizedUser()
    user = User.get(User.username == au.username)
    ...
    try:
        ...
        cfg = load_config()
        return render_template("courseTable.html", terms = terms,
                                                    isAdmin=au.user.isAdmin,
                                                    courseName_to_term = courseName_to_term,
                                                    program = program,
                                                    subject=subject,
                                                    cfg=cfg)
